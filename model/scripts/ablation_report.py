...
def _report_path(checkpoint_dir: Path, run_id: str, subdir: str = "yolo26") -> Path:
    return checkpoint_dir / subdir / run_id / "val_evaluation_report.json"
...
def _collect(run: dict, checkpoint_dir: Path, default_subdir: str = "yolo26") -> dict:
    ...
    if not rid:
        return out
    path = _report_path(checkpoint_dir, str(rid), run.get("subdir", default_subdir))
...
def build_report(manifest: dict, with_params: bool = False) -> str:
    ...
    default_subdir = manifest.get("subdir", "yolo26")
    rows = [_collect(run, checkpoint_dir, default_subdir) for run in manifest.get("runs", [])]
